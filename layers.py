...
def attention(query, key, value, mask=None, dropout=None):
    # Your code here
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1)) / torch.sqrt(d_k)  # Dot product and scaling
    ...
    if dropout is not None:
        p_attn = dropout(p_attn)  # Apply dropout if provided

    output = torch.matmul(p_attn, value)  # Compute weighted sum of values
