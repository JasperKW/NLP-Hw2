...
def beam_search_decode(model, src, src_mask, max_len, start_symbol, beam_size, end_idx):
    ...
    for i in range(max_len - 1):
        ...
        beam_indices = (top_indices // prob.size(1)).torch.long()  # Replace with torch.divide(indices, vocab_size, rounding_mode='floor')
        token_indices = (top_indices % prob.size(1)).torch.long()  # Replace with torch.remainder(indices, vocab_size)
